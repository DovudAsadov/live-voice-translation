...
class TranslationService:
    """Enhanced translation service with Uzbek API integration"""
    ...
    def __init__(self):
        ...
        # Initialize Uzbek services
        self._init_uzbek_services()
    ...
    def _init_uzbek_services(self):
        """Initialize Uzbek-specific translation and TTS services"""
        self.tilmoch_token = getattr(Config, 'TILMOCH_TOKEN', None)
        self.aisha_api_key = getattr(Config, 'AISHA_API_KEY', None)
        
        # Language mapping for Uzbek APIs
        self.lang_map_uzbek = {
            'uz': 'uzn_Latn',  # Default Uzbek (Latin)
            'uz_cyrl': 'uzn_Cyrl',  # Uzbek (Cyrillic)
            'ru': 'rus_Cyrl',
            'en': 'eng_Latn',
            'kaa': 'kaa_Latn',  # Karakalpak (Latin)
            'kaa_cyrl': 'kaa_Cyrl'  # Karakalpak (Cyrillic)
        }
        
        # Reverse mapping for convenience
        self.lang_map_reverse = {v: k for k, v in self.lang_map_uzbek.items()}
        
        logger.info(f"Uzbek services initialized (Tilmoch: {self.tilmoch_token is not None}, Aisha: {self.aisha_api_key is not None})")
    ...
    def _should_use_uzbek_stt(self, language: str) -> bool:
        """Check if we should use Uzbek STT service"""
        return (language.lower() in ['uz', 'ru', 'en'] and 
                self.aisha_api_key is not None)
    
    def _transcribe_audio(self, audio_data, language: str) -> str:
        """Transcribe audio to text with Uzbek STT support"""
        try:
            # Use Uzbek STT for supported languages
            if self._should_use_uzbek_stt(language):
                return self._transcribe_with_uzbek_stt(audio_data, language)
    ...
    def _transcribe_with_uzbek_stt(self, audio_path: str, language: str) -> str:
        """Transcribe audio using Uzbek STT API"""
        try:
            url = "https://back.aisha.group/api/v1/stt/post/"
            
            headers = {
                'x-api-key': self.aisha_api_key
            }
            ...
            logger.error(f"Uzbek STT error: {e}")
            return ""
    
    def _should_use_uzbek_translation(self, lang_from: str, lang_to: str) -> bool:
        """Check if we should use Uzbek translation service"""
        return (self.tilmoch_token is not None and 
                (lang_from.lower() in ['uz', 'ru', 'en'] or 
                 lang_to.lower() in ['uz', 'ru', 'en']))
    
    def _translate_text(self, text: str, lang_from: str, lang_to: str) -> str:
        """Translate text between languages with Uzbek translation support"""
        if lang_from == lang_to:
            return text
        
        try:
            # Use Uzbek translation service for supported languages
            if self._should_use_uzbek_translation(lang_from, lang_to):
                return self._translate_with_uzbek_api(text, lang_from, lang_to)
            
            # Fallback to DeepL
            if self.translator:
                target_lang = "EN-US" if lang_to == "en" else lang_to.upper()
    ...
    def _translate_with_uzbek_api(self, text: str, lang_from: str, lang_to: str) -> str:
        """Translate using Uzbek Tilmoch API"""
        try:
            url = "https://websocket.tahrirchi.uz/translate-v2"
            
            # Map language codes to Uzbek API format
            source_lang = self.lang_map_uzbek.get(lang_from.lower(), lang_from)
            target_lang = self.lang_map_uzbek.get(lang_to.lower(), lang_to)
            
            headers = {
                "Content-Type": "application/json",
                "Authorization": self.tilmoch_token
            }
            ...
            data = {
                ...
                "target_lang": target_lang,
                "model": "sayqalchi"  # Use the more advanced model
            }
            ...
            logger.error(f"Uzbek translation error: {e}")
            return text
    ...
    def _should_use_uzbek_tts(self, language: str) -> bool:
        """Check if we should use Uzbek TTS service"""
        return (language.lower() in ['uz', 'ru', 'en'] and 
                self.aisha_api_key is not None)
    
    def _text_to_speech(self, text: str, language: str) -> bytes:
        """Convert text to speech with Uzbek TTS support"""
        try:
            # Use Uzbek TTS for supported languages
            if self._should_use_uzbek_tts(language):
                return self._uzbek_tts(text, language)
            
            # Fallback to original async methods
    ...
    def _uzbek_tts(self, text: str, language: str) -> bytes:
        """Generate speech using Uzbek TTS API"""
        try:
            ...
            # Choose appropriate model and mood
            model = 'gulnoza' if language.lower() == 'uz' else 'jaxongir'
            ...
            logger.error(f"Uzbek TTS error: {e}")
            return b""
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'uzbek_translation_available': self.tilmoch_token is not None,
            'uzbek_tts_available': self.aisha_api_key is not None,
